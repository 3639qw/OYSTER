...
function translatePwd()
{
  ...
  if (seed.length > 0) 
  {
    if (pwd.length > 0) 
    {
      var results = "암호 변환 결과 <p><table cellpadding=5 cellspacing=0 id='resultTable'><tr>";
      var password = "";
      ...
      for (i=0; i<pwd.length; i++) {
        ...
        if (garbageIndex != -1) {
          ...
          results += "<br><font color='B60000'>" + garbageTable[garbageIndex] + "</font>";
        }
        
        results += "</td>";
      }
      results += "</tr></table></p><p>Password: <span style='font-family:Courier; color:#B60000; font-size:15pt'>" + password;

      chartedResults.innerHTML = results;
    }
    else
      chartedResults.innerHTML = "<font color='#B60000'>차트를 사용하여 변환할 암호를 입력하세요</font>";
  }
  else if (pwd.length == 0)
    chartedResults.innerHTML = "<font color='#B60000'>차트를 만들 단어와 변환할 암호를 입력하세요</font>";
  else
    chartedResults.innerHTML = "<font color='#B60000'>차트를 만들 단어와 변환할 암호를 입력하세요</font>";
