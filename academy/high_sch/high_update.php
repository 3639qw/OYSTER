...
$sql = "select * from high_sch where initial = '$id';";
...
<div style="height: 375px; width: 500px; margin-left: 30px; margin-top: 30px;">
    ...
    <div class="box insert_box" style="height: 100%; width: 100%; font-weight: normal;">
        ...
        <form name="high_insert" action="update_action.php" method="post" id="high_sch">
            <table border="0" align="center" style="border-collapse: collapse;">
                <tr>
                    <td>이니셜: <input type="text" class="" size="7" style="border: none; background: transparent;" name="initial" readonly> </td>
                ...
                <tr>
                    ...
                    <td>
                        학교특성<br><select class="form_field" name="type">
                ...
                <tr>
                    ...
                    <td rowspan="2" align="center" style="">
                        ...
                        <button type="button" class="button" onclick="location.href='high_delete.php?id=<?=$id?>'" style="height: 30px; width: 85px; margin-top: 15px; font-size: 13px;">삭제</button><br>
                        <button type="button" class="button" onclick="location.href='/academy/high_sch'" style="height: 30px; width: 85px; margin-top: 15px; font-size: 13px;">뒤로가기</button>
                    </td>
                </tr>
                <tr>
    ...
    <script>
        document.high_insert.initial.value="<?=$row['initial']?>"
        document.high_insert.name.value="<?=$row['name']?>"
        document.high_insert.gender.value="<?=$row['gender']?>";
        document.high_insert.founder.value="<?=$row['founder']?>";
        document.high_insert.est.value="<?=$row['est']?>";
        document.high_insert.office_edu.value="<?=$row['office_edu']?>";
        document.high_insert.type.value="<?=$row['type']?>";
        document.high_insert.zip.value="<?=$row['zip']?>";
        document.high_insert.addr1.value="<?=$row['addr1']?>";
        document.high_insert.addr2.value="<?=$row['addr2']?>";
