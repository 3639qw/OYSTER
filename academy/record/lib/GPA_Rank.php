...
class GPA_Rank extends Dictionary
{
    // 가중치 변환점수 GPA를 기준으로 산정한 석차연명부

    // 평점평균 석차연명부 (계열석차, 전체석차)
    // 졸업자, 졸업예정자 누적석차
    // $year: 선택한 학기재적년도
    // $type: 계열
    // $initial: 재적학교 이니셜
    ...
    // $isGPA: Y일경우 GPA기준 N일경우 환산점수 기준
    function Ranking($year,$type,$initial,$isNoN,$isType,$isgpa){
        ...
        // 통계자료
        // 20. 상위 20%, 40%, 60%, 80%
        // 21. 등급별로 해당 등위에 해당하는 GPA 찾기

        $record['document'] = 'GPA 석차연명부';
        if($isNoN == 'Y'){
            $record['target'] = '졸업자';
        }else if($isNoN == 'N'){
            $record['target'] = '졸업예정자';
        ...
        foreach ($stuid as $key => $value){
            ...
            if($isgpa == 'Y'){
                $raw = $ac_record->custom_record->Course_GP($key,$record['sch'],$record['initial']);
            }else if($isgpa == 'N'){
                $raw = $ac_record->custom_record->Course_Convertion($key,$record['sch'],$record['initial']);
            }else{
                $raw = $ac_record->custom_record->Course_GP($key,$record['sch'],$record['initial']);
            }
        ...
        // 8. 7에서 산출한 석차의 동석차 인원수를 산출한다
        $record['Tie']['1']['Rank_Tie'] = array_count_values($record['Rank']);
        foreach ($record['Tie']['1']['Rank_Tie'] as $key => $value){
            if($value == 1){
                unset($record['Tie']['1']['Rank_Tie'][$key]);
            }else{
                $record['Tie']['1']['Tie_sum'] += $value;
            }
        }
        ksort($record['Tie']['1']['Rank_Tie']);
        ...
        foreach ($record['Rank'] as $key => $value){
            ...
            if($record['Tie']['1']['Rank_Tie'][$value] >= 2){
                $record['Tie']['1']['Tie_Raw'][$value][$key] = $record['Raw']['Merged'][$key];
            }
        }
        ksort($record['Tie']['1']['Tie_Raw']);

        // 총점의 합을 기준으로 석차를 산정하고 이를 전체석차에 적용한다
        foreach ($record['Tie']['1']['Tie_Raw'] as $key => $value){
            foreach ($ac_record->statistics->HIGH_Rank($record['Tie']['1']['Tie_Raw'][$key]) as $k => $v){
                // 인수 풀이
        ...
        $record['Tie']['2']['Rank_Tie'] = array_count_values($record['Rank']);
        foreach ($record['Tie']['2']['Rank_Tie'] as $key => $value){
            if($value == 1){
                unset($record['Tie']['2']['Rank_Tie'][$key]);
            }else{
                $record['Tie']['2']['Tie_sum'] += $value;
            }
        }
        ksort($record['Tie']['2']['Rank_Tie']);

        foreach ($record['Rank'] as $key => $value){
            if($record['Tie']['2']['Rank_Tie'][$value] >= 2){
                $record['Tie']['2']['Tie_Raw'][$value][$key] = $record['Raw']['3'][$key];
            }
        }
        ksort($record['Tie']['2']['Tie_Raw']);

        foreach ($record['Tie']['2']['Tie_Raw'] as $key => $value){
            foreach ($ac_record->statistics->HIGH_Rank($record['Tie']['2']['Tie_Raw'][$key]) as $k  => $v){
                $record['Tie']['2']['Tie_Rank'][$key][$k] = $v;
                $record['Rank'][$k] += $v-1;
            }
        }
        ...
        $record['Tie']['3']['Rank_Tie'] = array_count_values($record['Rank']);
        foreach ($record['Tie']['3']['Rank_Tie'] as $key => $value){
            if($value == 1){
                unset($record['Tie']['3']['Rank_Tie'][$key]);
            }else{
                $record['Tie']['3']['Tie_sum'] += $value;
            }
        }
        ksort($record['Tie']['3']['Rank_Tie']);

        foreach ($record['Rank'] as $key => $value){
            if($record['Tie']['3']['Rank_Tie'][$value] >= 2){
                $record['Tie']['3']['Tie_Raw'][$value][$key] = $record['Raw']['2'][$key];
            }
        }
        ksort($record['Tie']['3']['Tie_Raw']);

        foreach ($record['Tie']['3']['Tie_Raw'] as $key => $value){
            foreach ($ac_record->statistics->HIGH_Rank($record['Tie']['3']['Tie_Raw'][$key]) as $k  => $v){
                $record['Tie']['3']['Tie_Rank'][$key][$k] = $v;
                $record['Rank'][$k] += $v-1;
            }
        }
        ...
        $record['Tie']['4']['Rank_Tie'] = array_count_values($record['Rank']);
        foreach ($record['Tie']['4']['Rank_Tie'] as $key => $value){
            if($value == 1){
                unset($record['Tie']['4']['Rank_Tie'][$key]);
            }else{
                $record['Tie']['4']['Tie_sum'] += $value;
            }
        }
        ksort($record['Tie']['4']['Rank_Tie']);

        foreach ($record['Rank'] as $key => $value){
            if($record['Tie']['4']['Rank_Tie'][$value] >= 2){
                $record['Tie']['4']['Tie_Raw'][$value][$key] = $record['Raw']['1'][$key];
            }
        }
        ksort($record['Tie']['4']['Tie_Raw']);

        foreach ($record['Tie']['4']['Tie_Raw'] as $key => $value){
            foreach ($ac_record->statistics->HIGH_Rank($record['Tie']['4']['Tie_Raw'][$key]) as $k  => $v){
                $record['Tie']['4']['Tie_Rank'][$key][$k] = $v;
                $record['Rank'][$k] += $v-1;
            }
        }
...
//        unset($record['Tie']);
        ...
        foreach ($record['Rank'] as $key => $value){
            if(round($record['sum']*0.2) == $value){
                $record['statistics']['20']['Rank'] = round($record['sum']*0.2);
                $record['statistics']['20']['GPA'] = $record['Average'][$key];
            }
            if(round($record['sum']*0.4) == $value){
                $record['statistics']['40']['Rank'] = round($record['sum']*0.4);
                $record['statistics']['40']['GPA'] = $record['Average'][$key];
            }
            if(round($record['sum']*0.6) == $value){
                $record['statistics']['60']['Rank'] = round($record['sum']*0.6);
                $record['statistics']['60']['GPA'] = $record['Average'][$key];
            }
            if(round($record['sum']*0.8) == $value){
                $record['statistics']['80']['Rank'] = round($record['sum']*0.8);
                $record['statistics']['80']['GPA'] = $record['Average'][$key];
            }
        }
